// ver todos:  Veo todos los puestos que no están borrados, esta función se invoca al cargar la página
const todos = () => {
  // cambio el título a Agregar y ver puestos
  let titulo = document.querySelector("#titPuestos");
  titulo.innerHTML = "Agregar y ver puestos";
  ...
  // tengo que volver a habilitar el boton Guardar, el boton Inicio y el de papelera
  ...
  db.collection("puestos")
    .orderBy("nombre") // asi ordeno por mercado, agregando orderBy('campo')
...
// agregar puestos
const guardar = () => {
  // si el campo nombre es nulo no dejamos agregar puestos
  ...
  if (nombre.length != 0) {
    ...
    db.collection("puestos") // "mercadería" es el nombre de la colección
      .add({
      ...
      .then((docRef) => {
        ...
        let puesto = (document.getElementById("puesto").value = "");
        Toastify({
          text: "El puesto se agregó correctamente",
          duration: 3000,
          destination: "",
          newWindow: true,
          close: true,
...
// Eliminar puestos de la papelera de reciclaje
const eliminar = (id) => {
  db.collection("puestos")
    ...
    .then(() => {
      ...
      Toastify({
        text: "El puesto se eliminó de la papelera",
        duration: 3000,
        destination: "",
        newWindow: true,
        close: true,
...
// borrar puestos, solo debo editar el campo "borrar" y cambiar el "N" por "S"
const borrar = (id) => {
  db.collection("puestos")
    .doc(id)
    .update({
    ...
    .then(() => {
      ...
      Toastify({
        text: "El puesto se envió a la papelera",
        duration: 3000,
        destination: "",
        newWindow: true,
        close: true,
...
// reciclar puestos, solo debo editar el campo "borrar" y cambiar el "S" por "N"
const reciclar = (id) => {
  db.collection("puestos")
    .doc(id)
    .update({
    ...
    .then(() => {
      ...
      Toastify({
        text: "El puesto se recicló de la papelera",
        duration: 3000,
        destination: "",
        newWindow: true,
        close: true,
...
const editar = (id, nombre, razonSocial, nave, puesto) => {
  // cambio el título de Agregar por Editar
  let titulo = document.querySelector("#titPuestos");
  titulo.innerHTML = "Editar Puestos";
  ...
  document.getElementById("nave").value = nave;
  ...
  boton.onclick = () => {
    ...
    let editar = db.collection("puestos").doc(id);
    ...
    return editar
      ...
      .then(() => {
        Toastify({
          text: "El puesto se editó correctamente",
          duration: 3000,
          destination: "",
          newWindow: true,
          close: true,
